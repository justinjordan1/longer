...
export default function PostInteractions(props: Props) {
  const { postId, upCount, downCount, myVote, comments, myReports, isMod, currentFlag, modReports, canInteract } = props
  ...
  const replyWords = countWords(replyDraft)
  const replyMeets = replyWords >= COMMENT_MIN_WORDS
  const replyPct = Math.min(100, (replyWords / COMMENT_MIN_WORDS) * 100)

  const submitReply = (parentId: string) => {
    if (!replyMeets || pending) return
    setReplyError('')
    startTransition(async () => {
      const res = await createComment(postId, replyDraft.trim(), parentId)
      if (res?.error) setReplyError(res.error)
      else { setReplyDraft(''); setReplyingTo(null); router.refresh() }
    })
  }
    ...
    <>
      {canInteract ? (
        ...
        <div className="panel" style={{ marginBottom: 18 }}>
          ...
          <div className="panel-body" style={{ display: 'flex', gap: 16, alignItems: 'center', flexWrap: 'wrap', fontSize: 13 }}>
            <span><span className="positive">↑{upCount}</span> up</span>
            <span><span className="accent">↓{downCount}</span> down</span>
            <span className="muted">› <Link href="/login" className="link">sign in/up</Link> to vote.</span>
          </div>
        </div>
      )}
        ...
        <div className="panel-body" style={{ padding: 18 }}>
          {comments.length === 0 && (
            <p className="muted" style={{ fontStyle: 'italic', margin: '0 0 16px' }}>
              › no responses yet. {canInteract ? 'add the first considered reply.' : 'sign in/up to add the first considered reply.'}
            </p>
          )}
          {(() => {
              ...
              return (
                <div key={c.id} className="comment-block">
                  <div className="muted" style={{ fontSize: 11.5, marginBottom: 4 }}>
                    {String(i + 1).padStart(3, '0')} · <Link href={`/u/${c.handle}`} style={{ color: 'inherit', textDecoration: 'underline', textUnderlineOffset: 3 }}><span className="accent">@</span>{c.handle}</Link> · {timeAgo(c.created_at)} ago · {c.word_count} wd
                  </div>
                  <div style={{ fontSize: 13.5, lineHeight: 1.65, marginBottom: 6 }}>{c.body}</div>
                  {canInteract && (
                    <div style={{ fontSize: 11.5 }}>
                      {replyingTo === c.id ? (
                        <button className="link muted" onClick={cancelReply} disabled={pending}>cancel reply</button>
                      ) : (
                        <button className="link muted" onClick={() => { setReplyingTo(c.id); setReplyDraft(''); setReplyError('') }} disabled={pending}>↳ reply</button>
                      )}
                    </div>
                  )}

                  {replies.length > 0 && (
                    <div style={{ marginTop: 12, marginLeft: 18, paddingLeft: 14, borderLeft: '1px dashed var(--rule)' }}>
                      {replies.map((r, ri) => (
                        <div key={r.id} style={{ marginBottom: 12 }}>
                          <div className="muted" style={{ fontSize: 11.5, marginBottom: 4 }}>
                            ↳ {String(ri + 1).padStart(2, '0')} · <Link href={`/u/${r.handle}`} style={{ color: 'inherit', textDecoration: 'underline', textUnderlineOffset: 3 }}><span className="accent">@</span>{r.handle}</Link> · {timeAgo(r.created_at)} ago · {r.word_count} wd
                          </div>
                          <div style={{ fontSize: 13.5, lineHeight: 1.65 }}>{r.body}</div>
                        </div>
                      ))}
                    </div>
                  )}

                  {canInteract && replyingTo === c.id && (
                    <div style={{ marginTop: 12, marginLeft: 18, paddingLeft: 14, borderLeft: '2px solid var(--accent)' }}>
                      <p className="smallcaps muted" style={{ fontSize: 11, margin: '0 0 6px' }}>
                        replying to <span className="accent">@</span>{c.handle}
                      </p>
                      <div className="field" style={{ marginBottom: 8 }}>
                        <textarea
                          autoFocus
                          ...
                          placeholder={`a reply. minimum ${COMMENT_MIN_WORDS} words.`}
                          style={{ minHeight: 100, resize: 'vertical' }}
                          disabled={pending}
                        />
                      </div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 4 }}>
                        <span className="muted" style={{ fontSize: 11.5 }}>word count</span>
                        <span style={{ fontVariantNumeric: 'tabular-nums', fontSize: 12, color: replyMeets ? 'var(--positive)' : 'var(--ink-soft)' }}>
                          {replyWords} <span className="muted">/ {COMMENT_MIN_WORDS}</span>
                        </span>
                      </div>
                      <div className="progress-track" style={{ marginBottom: 8 }}>
                        <div className={`progress-fill ${replyMeets ? 'ok' : ''}`} style={{ width: `${replyPct}%` }} />
                      </div>
                      {replyError && (
                        <p className="accent" style={{ fontSize: 12, marginBottom: 8 }}>! {replyError}</p>
                      )}
                      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 6 }}>
                        <button className="btn btn-ghost" onClick={cancelReply} disabled={pending}>cancel</button>
                        <button className="btn" disabled={!replyMeets || pending} onClick={() => submitReply(c.id)}>
                          {pending ? 'sending...' : replyMeets ? '▸ post reply' : `${COMMENT_MIN_WORDS - replyWords} more words`}
                        </button>
                      </div>
                    </div>
                  )}
          ...
          {canInteract ? (
            ...
            <div style={{ borderTop: '1px dashed var(--rule)', paddingTop: 18, marginTop: 4, fontSize: 13 }}>
              <span className="muted">› </span>
              <Link href="/login" className="link">sign in/up</Link>
              <span className="muted"> to add a response.</span>
            </div>
          )}
        </div>
      </div>
